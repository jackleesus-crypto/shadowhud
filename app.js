...
const state = {
  ...
  achievements: [
    {id:uuid(), name:'First 10', req:{completed:10}, owned:false}
  ],
...
function checkTitles(){
  var hardDone = state.quests.filter(function(q){ return q.difficulty==='Hard' && q.status==='done'; }).length;
  state.titles.forEach(function(t){
    var req = t.req || {}; var ok = true;
...
function renderCharacterHeader(){
  ...
  var badge = document.getElementById("rankBadge"); if (badge) badge.textContent = rnk;
  var rt = document.getElementById("rankText2"); if (rt) rt.textContent = rnk;
...
function renderAchievements(){
  var host = document.getElementById('achieveList'); if(!host) return;
  host.innerHTML = '';
  (state.achievements||[]).forEach(function(a){
...
document.addEventListener('click', function(e){
  var addTitle = e.target.closest && e.target.closest('#addTitleBtn');
  if(addTitle){
    var name = document.getElementById('newTitleName').value.trim();
    var key = document.getElementById('newTitleReqKey').value;
    var val = Number(document.getElementById('newTitleReqVal').value)||1;
    if(name){
      state.titles.push({id:uuid(), name:name, req:{[key]:val}, owned:false, equipped:false});
      save(); checkTitles(); renderTitles();
      document.getElementById('newTitleName').value='';
    }
  }
  var addAch = e.target.closest && e.target.closest('#addAchBtn');
  if(addAch){
    var nameA = document.getElementById('newAchName').value.trim();
    var keyA = document.getElementById('newAchReqKey').value;
    var valA = Number(document.getElementById('newAchReqVal').value)||1;
    if(nameA){
      state.achievements.push({id:uuid(), name:nameA, req:{[keyA]:valA}, owned:false});
