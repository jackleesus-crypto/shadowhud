...
// ---- Helpers ----
function uuid(){ try{ if(window.crypto && crypto.randomUUID) return uuid(); }catch(e){}; return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,function(c){var r=Math.random()*16|0,v=c==='x'?r:(r&0x3|0x8);return v.toString(16);}); }
// ---- State ----
const state = {
  gold: 0,
  level: 1,
...
function rankForLevel(lv){ if(lv<15)return'E'; if(lv<30)return'D'; if(lv<45)return'C'; if(lv<60)return'B'; if(lv<80)return'A'; return'S'; }
...
// Titles
function checkTitles(){var hardDone=state.quests.filter(function(q){return q.difficulty==='Hard'&&q.status==='done';}).length;state.titles.forEach(function(t){var req=t.req||{},ok=true;if(req.completed!=null)ok&=(state.journey.completed>=req.completed);if(req.dailyStreak!=null)ok&=(state.journey.streak>=req.dailyStreak);if(req.hardDone!=null)ok&=(hardDone>=req.hardDone);if(req.level!=null)ok&=(state.level>=req.level);t.owned=!!ok||!!t.owned;});}
...
function renderCharacterHeader(){
  ...
  var rnk=rankForLevel(state.level);
  var rb=document.getElementById("rankBadge"); if(rb) rb.textContent=rnk;
  var rt2=document.getElementById("rankText2"); if(rt2) rt2.textContent=rnk;
...
// Store
function checkAchievements(){var hardDone=state.quests.filter(function(q){return q.difficulty==='Hard'&&q.status==='done';}).length;state.achievements.forEach(function(a){var req=a.req||{},ok=true;if(req.completed!=null)ok&=(state.journey.completed>=req.completed);if(req.dailyStreak!=null)ok&=(state.journey.streak>=req.dailyStreak);if(req.hardDone!=null)ok&=(hardDone>=req.hardDone);if(req.level!=null)ok&=(state.level>=req.level);a.owned=!!ok||!!a.owned;});}
function renderAchievements(){var host=document.getElementById('achieveList');if(!host)return;host.innerHTML='';state.achievements.forEach(function(a){var row=document.createElement('div');row.className='card row space-between';row.innerHTML='<div><b>'+a.name+'</b><div class="muted">'+(a.owned?'Achieved':'Locked')+'</div></div>';host.appendChild(row);});}
document.addEventListener('click',function(e){var tbtn=e.target.closest&&e.target.closest('#addTitleBtn');if(tbtn){var name=document.getElementById('newTitleName').value.trim();var key=document.getElementById('newTitleReqKey').value;var val=Number(document.getElementById('newTitleReqVal').value)||1;if(name){state.titles.push({id:uuid(),name:name,req:{[key]:val},owned:false,equipped:false});save();checkTitles();renderTitles();document.getElementById('newTitleName').value='';}}var abtn=e.target.closest&&e.target.closest('#addAchBtn');if(abtn){var nameA=document.getElementById('newAchName').value.trim();var keyA=document.getElementById('newAchReqKey').value;var valA=Number(document.getElementById('newAchReqVal').value)||1;if(nameA){state.achievements.push({id:uuid(),name:nameA,req:{[keyA]:valA},owned:false});save();checkAchievements();renderAchievements();document.getElementById('newAchName').value='';}}});
