...
function renderQuests(filter='all'){
  ...
  const list = state.quests.filter(q=>{
    ...
    // Default 'all' excludes completed quests
...
// Add title/achievement form listeners
document.addEventListener('click', (e)=>{
  if(e.target && (e.target.id==='addTitleBtn' or e.target.id==='addAchBtn')){ e.preventDefault?.(); }

  const addTitle = e.target.closest('#addTitleBtn');
  if(addTitle){
    ...
    if(name){
      state.titles.push({id:crypto.randomUUID(), name, req:{[key]:val}, owned:false, equipped:false});
      save(); checkTitles(); renderTitles();
      document.getElementById('newTitleName').value='';
    }
  }
  const addAch = e.target.closest('#addAchBtn');
  if(addAch){
    ...
    if(name){
      state.achievements.push({id:crypto.randomUUID(), name, req:{[key]:val}, owned:false});
