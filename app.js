...
function checkTitles(){
  state.titles.forEach(t=>{
    if(t.id==='starter' && state.journey.completed>=1) t.owned=true;
    if(t.id==='earlybird' && state.journey.streak>=3) t.owned=true;
    if(t.id==='grit'){
      const hardDone = state.quests.filter(q=>q.difficulty==='Hard' && q.status==='done').length;
      if(hardDone>=5) t.owned=true;
    }
  });
}
...
function renderCharacterHeader(){
  ...
  $("#apText").textContent = fmt(state.ap);
  $("#equippedTitle").textContent = equippedTitle();
  $("#levelText2").textContent = state.level;
  const rnk = rankForLevel(state.level);
  const rb = document.getElementById("rankBadge"); if(rb) rb.textContent = rnk;
  const rt2 = document.getElementById("rankText2"); if(rt2) rt2.textContent = rnk;
...
$$(".add-attr").forEach(b=>{
  b.addEventListener("click", ()=>{
    const att = b.dataset.attr;
    if(state.ap>0){
      state.ap -= 1;
      state.attributes[att] += 1;
      save(); renderCharacterHeader(); refreshRadar();
    }
  });
});
...
(function safeBoot(){
  try{
    ...
    if(!state.journey) state.journey = {completed:0, streak:0, lastDate:null};
    try{ ensureDailyQuests(); }catch(e){ console.error('ensureDailyQuests failed', e); }
    state.xpToNext = state.xpToNext || xpNeededFor(state.level||1);
    renderAll(); setCountdown(); scheduleMidnight();
  }catch(err){
    console.error(err);
    const o=document.createElement('div');o.style.cssText='position:fixed;inset:0;background:#0f1017;color:#e8e9ff;display:flex;align-items:center;justify-content:center;z-index:9999;padding:1rem;text-align:center';
    o.innerHTML='<div><div style="font-weight:900;font-size:1.2rem;margin-bottom:.5rem">ShadowHUD couldn\'t start</div><div class="muted" style="margin-bottom:1rem">'+(err.message||err)+'</div><button id="resetApp" class="btn btn-primary">Reset Data</button></div>';
    document.body.appendChild(o);
    document.getElementById('resetApp').addEventListener('click', ()=>{ try{localStorage.clear();}catch(e){} location.reload(); });
  }
})();
...
// Hidden reset: tap version text 5 times
